...
var require_lib = __commonJS({
  "node_modules/webpack-virtual-modules/lib/index.js"(exports, module) {
    ...
    var VirtualModulesPlugin2 = class {
      ...
      getModuleList(filter2 = ALL) {
        var _a, _b;
        let modules = {};
        const shouldGetStaticModules = filter2 === ALL || filter2 === STATIC;
        const shouldGetDynamicModules = filter2 === ALL || filter2 === DYNAMIC;
  ...
  "node_modules/fetch-blob/streams.cjs"() {
    ...
    if (!globalThis.ReadableStream) {
      try {
        ...
        const { emitWarning: emitWarning2 } = process2;
        try {
          process2.emitWarning = () => {
          };
          Object.assign(globalThis, __require("stream/web"));
          process2.emitWarning = emitWarning2;
        } catch (error) {
          process2.emitWarning = emitWarning2;
          throw error;
        }
...
var prepareSymfonyCommand = (command, inputs = [], options = {}) => {
  ...
  return [finalOptions.php, finalOptions.script, command].concat([...inputs, "-v"]).map((part) => `'${part}'`).join(" ");
};
var execSymfonyCommand = async (finalCommand) => {
  return new Promise((resolve, reject) => {
    child_process.exec(finalCommand, (error, stdout, stderr) => {
      if (error) {
        reject(
          new Error(esm_default`
...
var getTwigComponentConfiguration = async () => {
  return (await runSymfonyCommandJson("debug:config", [
    "twig_component",
    "--resolve-env"
  ]))["twig_component"];
};
var getPreviewHtml = async () => {
  return await runSymfonyCommand("storybook:generate-preview");
};
var injectPreviewHtml = (previewHtml, targetHtml) => {
  const previewDom = new jsdom.JSDOM(previewHtml);
...
// src/server/lib/preview-compiler-plugin.ts
var PLUGIN_NAME = "preview-plugin";
var PreviewCompilerPlugin = unplugin.createUnplugin(() => {
  return {
    name: PLUGIN_NAME,
    webpack(compiler) {
      compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
        HtmlWebpackPlugin__default.default.getHooks(compilation).afterTemplateExecution.tapPromise(
          PLUGIN_NAME,
          async (params) => {
            try {
              ...
              nodeLogger.logger.error(esm_default`
...
var import_webpack_virtual_modules = __toESM(require_lib());
...
var Body = class {
  constructor(body, {
    ...
    if (buffer.Buffer.isBuffer(body)) {
      stream2 = Stream__default.default.Readable.from(body);
    } else if (isBlob(body)) {
      stream2 = Stream__default.default.Readable.from(body.stream());
    }
    this[INTERNALS] = {
      body,
      stream: stream2,
...
// src/server/lib/symfony-api-http.ts
var API_BASE_URI = "/_storybook/api";
var prepareRequest = (host, endpoint) => {
  return `${host}${API_BASE_URI}${endpoint}`;
};
var getPreviewHtml2 = async () => {
...
var import_is_glob = __toESM(require_is_glob());
...
var isStream = (s2) => !!s2 && typeof s2 === "object" && (s2 instanceof Minipass || s2 instanceof Stream__default.default || isReadable(s2) || isWritable(s2));
var isReadable = (s2) => !!s2 && typeof s2 === "object" && s2 instanceof events.EventEmitter && typeof s2.pipe === "function" && // node core Writable streams have a pipe() method, but it throws
s2.pipe !== Stream__default.default.Writable.prototype.pipe;
var isWritable = (s2) => !!s2 && typeof s2 === "object" && s2 instanceof events.EventEmitter && typeof s2.write === "function" && typeof s2.end === "function";
var EOF = Symbol("EOF");
...
var realpathSync = fs2.realpathSync.native;
var defaultFS = {
  lstatSync: fs2.lstatSync,
  readdir: fs2.readdir,
  readdirSync: fs2.readdirSync,
  readlinkSync: fs2.readlinkSync,
  realpathSync,
  promises: {
...
var fsFromOption = (fsOption) => !fsOption || fsOption === defaultFS || fsOption === fs2__namespace ? defaultFS : {
  ...defaultFS,
  ...fsOption,
  promises: {
    ...defaultFS.promises,
...
var normalize = (s2) => {
  ...
  if (c)
    return c;
  const n = s2.normalize("NFKD");
  normalizeCache.set(s2, n);
  return n;
};
...
var normalizeNocase = (s2) => {
  ...
  if (c)
    return c;
  const n = normalize(s2.toLowerCase());
  normalizeNocaseCache.set(s2, n);
  return n;
};
...
var PathBase = class {
  ...
  #readdirAddChild(e2, c) {
    return this.#readdirMaybePromoteChild(e2, c) || this.#readdirAddNewChild(e2, c);
  }
  #readdirAddNewChild(e2, c) {
  ...
  #readdirMaybePromoteChild(e2, c) {
    for (let p = c.provisional; p < c.length; p++) {
      const pchild = c[p];
      const name = this.nocase ? normalizeNocase(e2.name) : normalize(e2.name);
      if (name !== pchild.#matchName) {
        continue;
      }
      return this.#readdirPromoteChild(e2, pchild, p, c);
    }
  }
  #readdirPromoteChild(e2, p, index, c) {
    const v = p.name;
...
var PathWin32 = class _PathWin32 extends PathBase {
   ...
   */
  getRootString(path3) {
    return path2.win32.parse(path3).root;
  }
  /**
   * @internal
   */
...
var PathScurryBase = class {
  ...
  resolve(...paths) {
    ...
    const cached = this.#resolveCache.get(r2);
    if (cached !== void 0) {
      return cached;
    }
    const result = this.cwd.resolve(r2).fullpath();
    this.#resolveCache.set(r2, result);
    return result;
  }
  /**
  ...
  resolvePosix(...paths) {
    ...
    const cached = this.#resolvePosixCache.get(r2);
    if (cached !== void 0) {
      return cached;
    }
    const result = this.cwd.resolve(r2).fullpathPosix();
    this.#resolvePosixCache.set(r2, result);
    return result;
  }
  /**
      ...
      while (!paused) {
        ...
        const onReaddir = (er, entries, didRealpaths = false) => {
          ...
          if (follow && !didRealpaths) {
            ...
            for (const e2 of entries) {
              if (e2.isSymbolicLink()) {
                promises.push(e2.realpath().then((r2) => r2?.isUnknown() ? r2.lstat() : r2));
              }
            }
            if (promises.length) {
  ...
  constructor(ignored, { nobrace, nocase, noext, noglobstar, platform = defaultPlatform2 }) {
    ...
    for (const ign of ignored) {
      ...
      for (let i2 = 0; i2 < mm.set.length; i2++) {
        ...
        const m2 = new Minimatch(p.globString(), mmopts);
...
var GlobUtil = class {
  ...
  async matchCheck(e2, ifDir) {
    ...
    return this.matchCheckTest(needStat ? await e2.lstat() : e2, ifDir);
  }
  matchCheckTest(e2, ifDir) {
    return e2 && (this.maxDepth === Infinity || e2.depth() <= this.maxDepth) && (!ifDir || e2.canReaddir()) && (!this.opts.nodir || !e2.isDirectory()) && !this.#ignored(e2) ? e2 : void 0;
  }
  matchCheckSync(e2, ifDir) {
    ...
    const needStat = e2.isUnknown() || this.opts.stat;
    return this.matchCheckTest(needStat ? e2.lstatSync() : e2, ifDir);
  }
...
var computeAdditionalWatchPaths = (paths, baseDir) => {
  ...
  paths.map((v) => path2.join(baseDir, v)).forEach((watchPath) => {
    if ((0, import_is_glob.default)(watchPath)) {
      result.files.push(
        ...glob.sync(watchPath, {
          dot: true,
          absolute: true
    ...
    } else if (fs2__namespace.default.existsSync(watchPath)) {
      const stats = fs2__namespace.default.lstatSync(watchPath);
      (stats.isDirectory() ? result.dirs : result.files).push(watchPath);
    } else {
      nodeLogger.logger.warn(esm_default`
...
var DevPreviewCompilerPlugin = unplugin.createUnplugin((options) => {
  ...
  return {
    ...
    webpack(compiler) {
      const v = new import_webpack_virtual_modules.default();
      v.apply(compiler);
      let previewHtml = "";
      compiler.hooks.watchRun.tapPromise(PLUGIN_NAME2, async () => {
        previewHtml = (await getPreviewHtml2()).content;
        v.writeModule(
          "./symfony-preview.js",
          esm_default`
...
var extractComponentsFromTemplate = (source) => {
  ...
  const lookupComponents = (obj) => {
    return Object.entries(obj).reduce((names, [key, value]) => {
      if (value !== null && typeof value === "object") {
        ...
        for (const m2 of value.matchAll(functionRe)) {
          names.push([...m2][1]);
        }
      }
      if (tagRe.test(key)) {
  ...
  try {
    const documentObj = new import_fast_xml_parser.XMLParser().parse(`<div>${source}</div>`);
...
var TwigComponentResolver = class {
  ...
  resolveNameFromFile(file) {
    ...
    throw new Error(esm_default`Unable to determine template name for file "${file}":`);
  }
  resolveFileFromName(name) {
    ...
    for (const namespace in this.config.namespaces) {
      if ("" !== namespace && 0 === name.indexOf(namespace)) {
        lookupPaths.push(path2__default.default.join(this.config.namespaces[namespace], dirParts.slice(1).join("/")));
        break;
      }
    }
    if (this.config.namespaces[""] !== void 0) {
      lookupPaths.push(path2__default.default.join(this.config.namespaces[""], dirParts.join("/")));
    }
    lookupPaths.push(path2__default.default.join(this.config.anonymousTemplateDirectory, dirParts.join("/")));
    try {
      return __require.resolve(`./${filename}`, { paths: lookupPaths });
    } catch (err) {
      throw new Error(esm_default`Unable to find template file for component "${name}": ${err}`);
    }
  }
  resolvePhpFileFromName(name) {
  }
};
var PLUGIN_NAME3 = "twig-loader";
var TwigLoaderPlugin = unplugin.createUnplugin((options) => {
  ...
  return {
    ...
    transform: async (code, id) => {
      ...
      return esm_default`
            ...
            export default { 
                name: \'${name}\',
                hash: \`${crypto__default.default.createHash("sha1").update(code).digest("hex")}\`,
            }; 
           `;
    }
  };
...
var getBuildOptions = async (symfonyOptions) => {
  ...
  for (const { name_prefix: namePrefix, template_directory: templateDirectory } of Object.values(
    ...
    componentNamespaces[namePrefix] = path2.join(projectDir, "templates", templateDirectory);
  }
  const anonymousNamespace = path2.join(projectDir, "templates", twigComponentsConfig["anonymous_template_directory"]);
